...
const Home = () => {
  ...
  const tipoPesquisa = [
    { id: '0', descricao: 'Refeições' },
    { id: '1', descricao: 'Frigobar' },
  ];
  ...
  const Excel = async () => {
    ...
    const resp = await LancamentosAPI.excel(data);
    ...
    link.href = url;
    link.download = 'Lançamentos.xlsx';
  ...
  const AceitarTermo = async () => {
    ...
    data.CODCCU = filtros?.setor.toString();
    const resp = await LancamentosAPI.releases(data);
    if (resp?.data) {
      setDados(resp?.data);
    }
    ...
    return (
      <thead>
        <tr key={1} className="bg-cinza-claro ml-5 text-left">
          ...
          <th scope="col size120">Item</th>
          <th scope="col size120">Data lançamento</th>
          <th scope="col size120">Forma de identificação</th>
        </tr>
      </thead>
    );
  };
  ...
  const RenderDados = () => {
    ...
    return (
      ...
      dados.slice(pagesVisited, pagesVisited + usersPerPage).map((items, i) => {
        ...
        // const statusColor =
        //   items.usU_TPCAPT === 'Digitado' ? 'warning' : 'info';
        // const status =
        //   items.usU_TPCAPT === 'Digitado' ? 'Digitado' : 'Capturado';
        const number = +i;
        ...
        return (
          <tr key={number}>
            ...
            <td>{items.desref}</td>
            <td>
              {items.usU_DATCON && year !== 1901
                ? dayjs(items?.usU_DATCON).format('DD/MM/YYYY')
                : ''}{' '}
              {items.usU_HORCON > 0 ? FormataHora(items.usU_HORCON) : ''}
            </td>
